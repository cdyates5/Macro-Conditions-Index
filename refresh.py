...
def _find_ism_csv():
    """Locate the ISM history file, tolerating a flat repo layout."""
    raw = [os.path.join(HERE, "data", "ism.csv"),
           os.path.join(HERE, "ism.csv"),
           os.path.join(os.getcwd(), "data", "ism.csv"),
           os.path.join(os.getcwd(), "ism.csv")]
    cands = list(dict.fromkeys(os.path.normpath(p) for p in raw))
    for c in cands:
        if os.path.exists(c):
            return c
    return cands  # nothing found -> return the list so we can report it

ISM_CSV = _find_ism_csv()
...
# ------------------------------------------------------------------ ISM
def load_ism_csv():
    if isinstance(ISM_CSV, list):
        sys.exit("ERROR: ism.csv not found - it holds the ISM history seed.\n"
                 "Looked in:\n  " + "\n  ".join(ISM_CSV) +
                 "\nPut the file at either data/ism.csv or ism.csv next to refresh.py.")
    ism = {}
...
log(f"Loading ISM history ({os.path.relpath(ISM_CSV) if not isinstance(ISM_CSV, list) else 'ism.csv'})...")
ism = load_ism_csv()
